...
class MensajeForoRespuestaCreate extends FormularioAjax
{
     ...
     */
    private const FORM_ID = 'mensaje-foro-respuesta-create';
    private const FORM_NAME = 'Crear respuesta a mensaje de foro';
    private const TARGET_CLASS_NAME = 'MensajeForo';
    private const SUBMIT_URL = '/ses/mensajeforo/respuesta/create/';
    private const EXPECTED_SUBMIT_METHOD = FormularioAjax::HTTP_POST;
    private const ON_SUCCESS_EVENT_NAME = 'created.ses.mensajesforo.respuesta';
    private const ON_SUCCESS_EVENT_TARGET = '#mensaje-foro-ses-list';

    /**
     * Constructs the form object.
     */
    public function __construct($api = false, $padreId = null)
    {
    ...
    public function generateFormInputs(): string
    {
        ...
        $html = <<< HTML
        <input type="hidden" name="padreId" class="do-not-empty" value="$padreId">
    ...
    public function processSubmit(array $data = array()): void
    {
        ...
        // Comprobar el mensaje padre.

        $padreId = $data['padreId'] ?? null;

        if (! MensajeForo::dbExisteId($padreId)) {
            $errors[] = 'El mensaje al que estás respondiendo no existe.';
        }

        // Comprobar que el usuario en sesión tiene permiso para publicar en el foro.

        $foroId = MensajeForo::dbGet($padreId)->getId();
