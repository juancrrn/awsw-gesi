...
/**
 * Gesión del formulario de edición de una Asignacion.
 *
 * @package awsw-gesi
 * Gesi
...
{   
    ...
    public function __construct(string $action, Asignacion $asign) {
        parent::__construct('form-asignacion-editar', array('action' => $action));
        $this->asignacion = $asign;
        $this->id = $this->asignacion->getId();
    }
...
HTML;
    ...
    protected function procesa(array & $datos) : void
    {
        ...
        if (! Vista::hayMensajesError()) {
            ...
            $asignacion_id = $asignacion->dbActualizar();

            if ($asignacion_id) {
                Vista::encolaMensajeExito('Asignacion editada correctamente.', "/admin/asignaciones/");
            } else {
                Vista::encolaMensajeError('Hubo un error al editar la asignacion.');
            }
        }
