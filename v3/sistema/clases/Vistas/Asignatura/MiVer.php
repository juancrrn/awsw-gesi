...
class MiVer extends Modelo
{
	...
	public function procesaContent() : void
	{
        $nombre = $this->asignatura->getNombreCompleto();
		$curso = $this->asignatura->getCursoEscolarRaw();
		$corto = $this->asignatura->getNombreCorto();
		$asignatura_id = $this->asignatura->getId();
		$asignaciones = array();

		if(Asignacion::gpAsigAs($asignatura_id)){
			$asignaciones = Asignacion::dbGetAsig($asignatura_id);
		}
		...
		<header class="page-header">
			<p class="page-back-button"><a href="$url_volver">Volver a Asignaturas</a></p>
			<h1>$nombre (Curso $curso) ($corto)</h1>		
		</header>
...
HTML;   
        ...
        if(!empty($asignaciones)){
            foreach ($asignaciones as $a){
                $profesor_id = $a->getProfesor();
                $grupo_id = $a->getGrupo();
                $horario = $a->getHorario();

                if(Usuario::dbExisteId($profesor_id)){
